...
var loadAndStoreDocs = function(db, couchdb, concurrentDocLimit, docsToDownload) {
  if (docsToDownload.length) {
    ...
    return couchdb.allDocs({
      ...
      // Attempt to remove all variations of \u0000, including sitautions where
      // it's already attempted to be escaped by other escapers (eg \\u0000)
      // All slashes must be removed otherwise unremoved slashes could then cause
      // other syntax errors
      // Note that this will also unfortunately strip genuine backslashes which
      // precede instances of \u0000 - see:
      //   https://github.com/medic/medic-webapp/issues/4473
      insertSql = insertSql.replace(/(\\)+u0000/g, '');
