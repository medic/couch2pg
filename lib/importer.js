...
var getSeq = function(db, sourceId) {
  return new rsvp.Promise(function(resolve, reject) {
    var queryStmt = 'SELECT seq FROM couchdb_progress WHERE source_id = %L';
    db.one(format(queryStmt, sourceId))
    .then(function(result) { return resolve(result.seq); })
    .catch(function() {
      var insertStmt = 'INSERT INTO couchdb_progress(seq, source_id) VALUES (%L, %L)';
      db.query(format(insertStmt,  0, sourceId))
...
var storeSeq = function(db, seq, sourceId) {
  var sql = 'UPDATE couchdb_progress SET seq = %L WHERE source_id = %L';
  return db.query(format(sql, seq, sourceId));
};
...
var importChangesBatch = function(db, couchdb, concurrentDocLimit, changesLimit, sourceId) {
  return getSeq(db, sourceId)
    .then(function(seq) {
...
module.exports = function(db, couchdb, concurrentDocLimit, changesLimit, sourceId) {
  ...
  var importLoop = function(accChanges) {
    ...
    return importChangesBatch(db, couchdb, concurrentDocLimit, changesLimit, sourceId)
    .then(function(changes) {
      return storeSeq(db, changes.lastSeq, sourceId).then(function() {
  ...
  return {
    ...
    */
    importBatch: function() { return importChangesBatch(db, couchdb, concurrentDocLimit, changesLimit, sourceId); },
    /*
      Persists a given seq value.
    */
    storeSeq: function(seq) { return storeSeq(db, seq, sourceId); }
  };
};
